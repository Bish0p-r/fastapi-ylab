...
MenuWithThisTitleExists = HTTPException(
    status_code=status.HTTP_409_CONFLICT, detail='A menu with this title already exists.'
)
...
SubMenuWithThisTitleExists = HTTPException(
    status_code=status.HTTP_409_CONFLICT, detail='A submenu with this title already exists.'
)
...
DishWithThisTitleExists = HTTPException(
    status_code=status.HTTP_409_CONFLICT, detail='A dish with this title already exists.'
)
