...
class SubMenuRepository(BaseRepository):
    ...
    @classmethod
    async def get_one_or_none_with_counts(cls, session: AsyncSession, menu_id: UUID, submenu_id: UUID, **filter_by):
        query = (
            (
    ...
    @classmethod
    async def get_all_with_counts(cls, session: AsyncSession, menu_id: UUID, **filter_by):
        query = (
            select(SubMenu.__table__.columns, func.count(distinct(Dish.id)).label("dishes_count"))
            .outerjoin(Dish, SubMenu.id == Dish.submenu_id)
            .filter(SubMenu.menu_id == menu_id)
